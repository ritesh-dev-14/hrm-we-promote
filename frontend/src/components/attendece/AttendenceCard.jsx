...
export default function AttendanceCard() {
  ...
  useEffect(() => {
    ...
    // IF DAY COMPLETED -> USE FINAL TIME
    ...
    if (savedData.startTime) {
      ...
      if (savedData.status === "break" && savedData.breakStartedAt) {
        ...
        setSeconds(totalWorked - currentBreak);
      } else {
        setSeconds(totalWorked);
      }
    }
  }, [today]);
  ...
  const handleClockIn = () => {
    ...
    // prevent second clock in same day
  ...
  const handleBreakToggle = () => {
    ...
    // RESUME WORK
      ...
      const breakSeconds = Math.floor((now - existing.breakStartedAt) / 1000);
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          status: "working",
          breakStartedAt: null,
          breakDuration: (existing.breakDuration || 0) + breakSeconds,
        }),
      );
          ...
          {status !== "idle" && (
            ...
            >
              <div
                ...
                ${status === "working" ? "bg-emerald-500" : "bg-amber-500"}`}
              />

              <span
                className={`text-[11px] font-bold uppercase tracking-wider
                ${
                  status === "working" ? "text-emerald-700" : "text-amber-700"
                }`}
              >
        ...
        <AnimatePresence mode="wait">
          ...
          {status === "idle" && (
            <motion.button
              ...
              className="relative overflow-hidden flex-1 px-6 py-3 rounded-2xl text-sm font-semibold bg-linear-to-r from-indigo-600 to-violet-600 text-white shadow-xl hover:shadow-indigo-200"
            >
          ...
          ${
            status === "working"
              ...
              <motion.button
                ...
                className="flex-1 px-6 py-3 rounded-2xl text-sm font-semibold text-white bg-linear-to-r from-rose-500 to-red-500 shadow-xl hover:shadow-rose-200"
              >
                <div className="flex items-center justify-center gap-2">
                  <LogOut size={16} />
                  End Workday
